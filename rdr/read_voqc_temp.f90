SUBROUTINE read_voqc_temp
 ...
 CHARACTER(LEN=50) :: fname ='../vobsyymmddhh'
...
!----------------------------------------------------------
 ...
 IF (.NOT.obs(1)%obs_is_allocated) THEN
    ...
    timing_id = 0
    IF (ltiming) CALL add_timing(timing_id,'allocate_read_vobs_temp')
 ...
 !
       ...
       IF (num_temp == 0 ) THEN
           IF(lprint_read) WRITE(6,*)'SKIP',cdate,ctime
          i = i - 1
          CLOSE(lunin)
          wdate = cdate
          wtime = ctime
       ...
       READ_STATION_OBS : DO k=1,num_temp
          ...
          IF(lprint_read) WRITE(6,*)istnr,lat,lon,ierr
          ...
          IF(lprint_read) WRITE(6,*)stations(istnr),use_stnlist
          IF(stations(istnr) == 0) THEN
             ...
             IF ( use_stnlist ) THEN
                ...
                ENDDO
                IF (lprint_read) WRITE(6,*)'STATION ',istnr,stat_i
          ...
          IF (lprint_read )WRITE(6,*)'FOUND',i,stat_i,istnr,cdate,ctime
          ...
          IF (lprint_read) WRITE(6,*)'CHECK:',val

          kk_lev = 0
          DO kkk=1,my_temp_lev
             IF (ABS(val(1) -lev_lst(kkk)) < 1.e-6 ) kk_lev = kkk
          ENDDO
          IF (lprint_read) WRITE(6,*)'CHECK:',kk_lev,val
          ...
          IF (lprint_read) WRITE(6,*)'ADD:',kk_lev,val
...
!         obs(stat_i)%o(i)%val(kk_lev+ my_temp_lev*(qq_ind-1)) = val(7) *1.e3
       ...
       IF(lprint_read) WRITE(6,*)'DONE',istnr
