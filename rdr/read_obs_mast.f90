SUBROUTINE read_obs_mast
 ...
 STATION_LOOP : DO istnr=1,max_flux_station
    ...
    fname = TRIM(obspath)//'Meas_'//TRIM(stname(istnr))//'_Flux.txt'
    ...
    FLUX_READ_LOOP : DO
       ...
       FLUX_SUB_LOOP : DO
          ...
          IF ( newdate < sdate )CYCLE FLUX_READ_LOOP
    ...
    fname = TRIM(obspath)//'Meas_'//TRIM(stname(istnr))//'_Mast.txt'
    OPEN(lunin,FILE=fname,STATUS='old',IOSTAT=ierr)
    IF ( ierr /= 0 ) THEN
       WRITE(6,*)'Could not open ',TRIM(fname)
       EXIT MAST_LOOP
    ...
    MAST_READ_LOOP : DO
       ...
       IF (gr_ind /= 0 .AND. (ABS(mast(6)-err_ind)>1.e-6)) obs(stat_i)%o(ii)%val(gr_ind) = mast(6)
