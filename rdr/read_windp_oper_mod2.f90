SUBROUTINE read_windp_oper_mod2
    ...
    EXP_LOOP : DO k=1,nexp,2
       ...
       READ_LOOP : DO 
          ...
          IF ( fc_ind /= 0 ) THEN
             ...
             IF (wt_ind /= 0) hir(i)%o(j)%nal(k    ,fc_ind,wt_ind) =  hir(i)%hgt
             IF (wt_ind /= 0) hir(i)%o(j)%nal(k+1  ,fc_ind,wt_ind) =  hir(i)%hgt
          ENDIF
...
END SUBROUTINE read_windp_oper_mod2
