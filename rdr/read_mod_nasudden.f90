...
!         ! Interpolate wind linear between leves
          w = (lev_lst(ll) - z(z_ind+1)) / (z(z_ind) -  z(z_ind+1)) 

          uu = u(z_ind) * ( 1-w) + u(z_ind+1) *w
          vv = v(z_ind) * ( 1-w) + v(z_ind+1) *w
          tt = t(z_ind) * ( 1-w) + t(z_ind+1) *w 
          ...
          WRITE(6,*)'JJ_SCALE',jj_scale

          IF (ff_ind /= 0) hir(i)%o(j)%nal(k,jj_scale,ll)  = uu
          IF (tt_ind /= 0) hir(i)%o(j)%nal(k,jj_scale,ll)  = tt
...
END SUBROUTINE read_mod_nasudden
