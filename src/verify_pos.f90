...
!
 ...
 INTEGER :: i,j,jj,k,l,m,n,o,oo,ii,aerr,                &
 ...
 STATION_CYCLE : DO i=1,maxstn
    ...
    J_CYCLE : DO j=1,hir(i)%ntim
     ...
     FC_CYCLE : DO n=1,nfclengths
       ...
       wtime = wtime / 10000
       time_pos = wdate * 100 + wtime

       ! Check if there is any observation available at this time

       IF ( UBOUND(obs(i)%pos,1) < time_pos    ) CYCLE J_CYCLE
       IF ( LBOUND(obs(i)%pos,1) > time_pos    ) CYCLE FC_CYCLE
       IF (        obs(i)%pos(time_pos) == 0 ) CYCLE FC_CYCLE

          jj = obs(i)%pos(time_pos)
