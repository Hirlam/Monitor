SUBROUTINE plot_scat_diff_new(lunout,nparver,nr,nrun,    &
 ...
 USE data, ONLY : obstype,expname,err_ind,nexp,station_name,csi, &
                  ...
                  yearvise_freq,monthvise_freq,monthvise
 USE mymagics
  ...
  DO j=1,nparver
   ...
   ENDIF
    ...
    ELSEIF(p1 < 999999 ) THEN
       pp1 = monincr(p1,monthvise_freq-1)
       IF(p1 == pp1 ) THEN
...
!       IF (kk.GT.0)THEN
!          miny =  MINVAL(yval(1:kk))
!          maxy =  MAXVAL(yval(1:kk))
!          minx =  MINVAL(xval(1:kk))
!          maxx =  MAXVAL(xval(1:kk))
!       ENDIF

!       IF (diff_ind.EQ.0) THEN
!        rtmp = MIN(miny,minx)        
!        miny = rtmp
!        minx = rtmp
!        rtmp = MAX(maxy,maxx)        
...
!       ENDIF
        ...
        IF ( nr /= 0 ) THEN
           CALL calc_corr(47,kk,nr,xval(1:kk),yval(1:kk),expname(jj),wtext)
        ENDIF
